      ...
      <div class = "container fit">
        <div class = "row vertical-align">
          <div class = "col-sm-8 specialBlue img-rounded">
            ...
            <p>The type of encryption KeepItSecret uses is similar to <a href = "https://en.wikipedia.org/wiki/Public-key_cryptography" target = "_blank">public key cryptography</a>, and so for public key cryptography to work, as the name suggests, keys would need to be generated. When you register an address and a password, they get stored in a database along with the date of creation and whether they have a hard expiration date. If the key has a hard expiration date, it will get deleted after one day of its creation regardless of usage. Otherwise, the key will get deleted after a week of disuse.</p>
            <h3>How It Works - <a href = "Encode/index.php" class = "linkGlyph">Encoding</a></h3>
            <p>Through encryption, a message goes through two main phases to make it unreadable. The first step taken is that the recipient's private key is found out from their public key. The user's private key is then used as the random number generator's (RNG's) seed. This is a necessary step due to how <a href = "https://en.wikipedia.org/wiki/Pseudorandom_number_generator" target = "_blank">pseudo-random number generation</a> works: the same seed will always generate the same "random" numbers (this is used later for decoding the message). Then, the message gets <a href = "https://en.wikipedia.org/wiki/Caesar_cipher" target = "_blank">Caeser Ciphered</a> with each shift amount being a random number for each letter. Then, each letter of the message gets shuffled. Afterwards, random letters are inserted in the message. Then, a shifted version of the original message's length is concatenated at the end of the message to be used for decrypting it.</p>
